...
public class SmsSuccessHandler implements AuthenticationSuccessHandler {
    ...
    /**
     * 这里使用全局TokenService工具类
     * 因为整合jwt使用的是DefaultTokenServices
     * 所以这里生成token也使用DefaultTokenServices
     * 使用AuthorizationServerTokenServices生成token，
     * 生成的是未整合jwt的token
     */
    private final @NonNull TokenServiceUtils tokenServiceUtils;
    ...
    public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response, Authentication authentication) throws IOException {
        ...
        // 2. 构建令牌请求, 因为自定义，grantType可以为null或任何字符串
        TokenRequest tokenRequest = new TokenRequest(new HashMap<>(0), clientId, clientDetails.getScope(), null);
        // 3. 创建 oauth2 令牌请求
