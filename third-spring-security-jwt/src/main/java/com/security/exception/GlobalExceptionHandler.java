...
public class GlobalExceptionHandler {
    ...
    /**
     * 注解权限开发抛出注解的异常，抛出 AccessDeniedException 异常，不会被 accessDeniedHandler 处理器捕获，而是会被全局异常捕获
     * @param e
     * @return
     */
    @ExceptionHandler(value = AccessDeniedException.class)
    @ResponseBody
    public ResponseResult exceptionHandler(HttpServletRequest req, AccessDeniedException e) {
        logger.error("全局异常捕获AccessDeniedException：{}", e.getLocalizedMessage());

        return ResponseResult.builder().code(CodeEnum.FORBIDDEN.getCode()).message(CodeEnum.FORBIDDEN.getMessage());
    }
